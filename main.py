...
def process_query(query: str):
    ...
    try:
        relevance_check = groq_client.chat.completions.create(model="llama-3.3-70b-versatile",
                                                              messages=[
        ...
        if relevance == "yes":
            response = groq_client.chat.completions.create(model="llama-3.3-70b-versatile", 
                                                        messages=[{"role": "system", "content": "You are CarbonGPT, an expert on carbon emissions. Answer only about carbon emissions."}, {"role": "user", "content": query_lower}], 
                                                        max_tokens=200)
            ...
            return {
                "status_code": 400,
                "response_content": f"Error: '{query}' is not related to carbon emissions."
            }
